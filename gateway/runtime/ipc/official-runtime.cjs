...
function sendTranscriptThreadStatuses(clientId, hub = wsHub, statusObserver = transcriptStatusObserver) {
  ...
  for (const message of statusObserver.replayMessages()) {
    if (message?.payload?.params?.status?.type === "idle") continue;
    const envelope = officialLiveObserverEnvelope(message);
    if (envelope && hub.sendTo(clientId, envelope, { suppressDiagnostic: true })) sent += 1;
  }
  return sent;
...
function authoritativeThreadListResponse(data, statusObserver) {
  ...
  return typeof data === "string" ? JSON.stringify(response) : response;
}

function authoritativeDirectResponse(channel, payload, requestSummary, statusObserver = transcriptStatusObserver) {
  if (channel !== MESSAGE_FOR_VIEW_CHANNEL || !payload || typeof payload !== "object") return payload;
  if (payload.method === "thread/status/changed") return null;
  if (!isStructuredThreadListResponse(channel, payload, requestSummary)) return payload;
  const message = authoritativeThreadListResponse(payload.message, statusObserver);
...
function routeOfficialWebContentsSend(
  ...
  const authoritativePayload = authoritativeDirectResponse(channel, payload, requestSummary);
  if (authoritativePayload === null) return true;
  if (authoritativePayload !== payload) {
