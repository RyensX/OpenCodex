...
test("transcript status observer discovers threads from current global state entries", (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "opencodex-transcript-global-state-"));
  const filePath = path.join(directory, "thread-global.jsonl");
  t.after(() => fs.rmSync(directory, { recursive: true }));
  fs.writeFileSync(
    filePath,
    `${JSON.stringify({ type: "event_msg", payload: { type: "task_started", turn_id: "turn-1" } })}\n`
  ...
  assert.equal(
    observer.observeSidebarBootstrap({
      globalStateEntries: [{ key: "pinned-thread-ids", value: ["thread-global"] }],
    }),
    1
  );
  assert.deepEqual(published.at(-1)?.payload.params.status, { type: "active", activeFlags: [] });
...
test("replays the latest transcript status only to the browser that completed hello", () => {
  ...
  assert.deepEqual(sent, [{
    ...
    envelope: {
      ...
      payload: transcriptObserver.replayMessages()[1].payload,
      args: [transcriptObserver.replayMessages()[1].payload],
    },
...
test("replays the Desktop unread atom without per-thread synthetic read states", () => {
  const sent = [];
  const transcriptObserver = {
    replayMessages: () => [{
      channel: "thread-status-evidence",
  ...
  assert.deepEqual(sent.map((envelope) => envelope.channel), ["persisted-atom-updated"]);
});
...
test("replaces stale direct thread list statuses and suppresses direct status notifications", () => {
  const statusObserver = {
    replayMessages: () => [{
      payload: {
        method: "thread/status/changed",
  ...
  const response = __test.authoritativeDirectResponse(
    "codex_desktop:message-for-view",
    {
      type: "mcp-response",
      message: {
  ...
  assert.deepEqual(response.message.result.data[0].status, { type: "idle" });
  assert.equal(
    __test.authoritativeDirectResponse(
      "codex_desktop:message-for-view",
      {
        type: "mcp-notification",
        method: "thread/status/changed",
        params: { threadId: "thread-idle", status: { type: "active", activeFlags: [] } },
      },
      null,
      statusObserver
    ),
