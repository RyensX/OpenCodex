...
OpenCodex itself is only a middleware layer for Codex Desktop. With Xvfb, it can run on a headless Linux server without a physical display.
...
Official Codex Desktop currently provides only macOS and Windows builds. The Linux build is generated by the community project [ilysenko/codex-desktop-linux](https://github.com/ilysenko/codex-desktop-linux), which builds automatically from the upstream macOS DMG:
...
OpenCodex expects `CODEX_DESKTOP_PATH` to point to this directory by default.
...
| `CODEX_DESKTOP_APP_PATH` | Codex Desktop installation directory containing `app.asar`. | `/tmp/codex-desktop-linux/codex-app` |
| `CODEX_DESKTOP_EXECUTABLE_PATH` | Electron executable path. | `/tmp/codex-desktop-linux/codex-app/codex` |
