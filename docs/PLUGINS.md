...
提交后的插件声明会进入正式 Modification Kernel：高级适配器按依赖展开，底层 Contribution 按适配器批量编译，并统一经历定位、应用、验证、激活和真实命中。一个插件修改点包含多个 Contribution 时，只有全部效果都真实发生才显示为已命中。
...
- 登录壳替换官方 Document 时，旧页面的插件 Kernel、挂载、Hook 和协议订阅会统一销毁；尚未完成的旧 generation 工厂不能提交到新页面。
